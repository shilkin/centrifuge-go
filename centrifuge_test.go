...
func (c *connectionMock) getConnectAck() (ack []byte) {
	clientID := libcentrifugo.ConnID("cda49f81-44fb-4857-4ec8-ccae2670589a")
	body := &libcentrifugo.ConnectBody{
		Client: &clientID,
	}
...
func newTestCentrifugeImpl(url, project string, creds *Credentials, events *EventHandler, config *Config, connMock connectionMock) *centrifugeImpl {
	return &centrifugeImpl{
		url:         url,
		...
		createConnection: connMock.initConnectionMock,
	}
...
func TestConnectOk(t *testing.T) {
	c := newTestCentrifugeImpl(url, project, testCredentials(), nil, DefaultConfig, connectionMock{})
	err := c.Connect()
	if err != nil {
...
func TestWriteTimeout(t *testing.T) {
	c := newTestCentrifugeImpl(url, project, testCredentials(), nil, DefaultConfig, connectionMock{errTimeout: true})
	err := c.Connect()
	if err == nil {
		t.Error("Should fail")
	}
...
func TestConnectionError(t *testing.T) {
	c := newTestCentrifugeImpl(url, project, testCredentials(), nil, DefaultConfig, connectionMock{errConn: true})
	err := c.Connect()
	if err == nil {
		t.Error("Should fail")
	}
...
func TestSubscriptionError(t *testing.T) {
	c := newTestCentrifugeImpl(url, project, testCredentials(), nil, DefaultConfig, connectionMock{errSub: true})
	err := c.Connect()
	if err != nil {
		t.Errorf("Should pass but error is '%s'", err)
	}
...
func jsonMessage(t *testing.T, msg string, positive bool) error {
	...
	c := newTestCentrifugeImpl(url, project, testCredentials(), events, DefaultConfig, connectionMock{IncomingMessages: messages})
	...
	incoming := false
	subEvents := &SubEventHandler{
		OnMessage: func(sub *Sub, msg libcentrifugo.Message) error {
